        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={i} className="bg-surface-lowest p-4 rounded-xl border border-border-dark flex flex-col gap-2 shadow-sm">
                ...
                <p className="text-xl font-black text-on-surface mt-1">{card.value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-surface-lowest rounded-2xl border border-border-dark overflow-hidden shadow-2xl">
                  ...
                  {isLoading ? (
                    ...
                    <tr key={s.id} className="hover:bg-white/[0.02] transition-colors group">
                      <td className="px-6 py-4">
                        <p className="text-sm font-bold text-on-surface leading-none">{s.name}</p>
                        <p className="text-[10px] text-text-muted mt-1 font-medium opacity-60">{s.companyName}</p>
                      </td>
                      <td className="px-6 py-4 text-sm text-on-surface">{s.contactPerson || '—'}</td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-on-surface">{s.country}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-text-muted">{s.paymentTerms || '—'}</td>
                      <td className="px-6 py-4 text-sm text-center font-bold text-on-surface">{s._count?.purchases ?? 0}</td>
                      <td className="px-6 py-4">
            ...
            <div className="bg-surface-container px-6 py-4 border-t border-border-dark flex items-center justify-between">
              <p className="text-xs text-[#92adc9]">Showing <span className="text-on-surface font-bold">{filteredSuppliers.length}</span> of <span className="text-on-surface font-bold">{suppliers.length}</span> suppliers</p>
            </div>
          </div>
        </div>
      ) : (
        <div className="max-w-4xl mx-auto bg-surface-lowest rounded-2xl border border-border-dark p-8 shadow-2xl space-y-10">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input type="text" className="bg-surface-high border-outline-variant text-on-surface text-sm rounded-xl w-full h-12 px-4 focus:ring-primary/40 focus:border-primary" placeholder="Common Name"
              ...
              <div className="space-y-2">
                ...
                <input type="text" className="bg-surface-high border-outline-variant text-on-surface text-sm rounded-xl w-full h-12 px-4 focus:ring-primary/40 focus:border-primary" placeholder="Legal Entity"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                ...
                <input type="text" className="bg-surface-high border-outline-variant text-on-surface text-sm rounded-xl w-full h-12 px-4 focus:ring-primary/40 focus:border-primary"
                  value={formData.contactPerson} onChange={(e) => setFormData({ ...formData, contactPerson: e.target.value })} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-text-muted uppercase tracking-[0.15em] ml-1">Email Address</label>
                <input type="email" className="bg-surface-high border-outline-variant text-on-surface text-sm rounded-xl w-full h-12 px-4 focus:ring-primary/40 focus:border-primary" placeholder="supplier@company.com"
                  value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-text-muted uppercase tracking-[0.15em] ml-1">Phone Number</label>
                <input type="tel" className="bg-surface-high border-outline-variant text-on-surface text-sm rounded-xl w-full h-12 px-4 focus:ring-primary/40 focus:border-primary" placeholder="+00 000 000 000"
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input type="text" className="bg-surface-high border-outline-variant text-on-surface text-sm rounded-xl w-full h-12 px-4 focus:ring-primary/40 focus:border-primary" placeholder="e.g. China, UAE, Germany"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <select className="bg-surface-high border-outline-variant text-on-surface text-sm rounded-xl w-full h-12 px-4 focus:ring-primary/40 focus:border-primary"
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <textarea className="bg-surface-high border-outline-variant text-on-surface text-sm rounded-xl w-full h-24 px-4 py-3 focus:ring-primary/40 focus:border-primary" placeholder="Any additional information..."

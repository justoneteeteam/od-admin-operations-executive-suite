    ...
    return (
        <div className="px-3 sm:px-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-on-surface-variant text-sm">Warehouse:</span>
                        <select
                            className="bg-surface-high border border-outline-variant text-on-surface text-sm rounded-lg p-2.5 focus:ring-primary focus:border-primary"
            ...
            {activeTab === 'overview' && (
                <>
                    {loading && !metrics ? (
                        <div className="text-on-surface">Loading Inventory Dashboard...</div>
                    ) : (
                        <>
                            {/* KPI Cards */}
                                ...
                                <div className="bg-surface-lowest p-4 sm:p-6 rounded-lg shadow-sm border border-border-dark h-72 sm:h-96">
                                    ...
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            ...
                                            <Tooltip contentStyle={{ backgroundColor: '#ffffff', borderColor: '#c3c6d7', color: '#191b23' }} />
                                ...
                                <div className="bg-surface-lowest p-4 sm:p-6 rounded-lg shadow-sm border border-border-dark">
                                    ...
                                    <div className="overflow-auto max-h-80 custom-scrollbar">
                                        <table className="w-full text-left">
                                            <thead>
                                                <tr className="text-on-surface-variant text-sm border-b border-border-dark">
                                            ...
                                            <tbody>
                                                {lowStockProducts.map(product => (
                                                    <tr key={product.id} className="border-b last:border-0 border-border-dark hover:bg-surface-container transition-colors">
                                                        <td className="py-2 text-sm font-medium text-on-surface">{product.name}</td>
                                                        <td className="py-2 text-sm text-on-surface-variant">{product.sku}</td>
                                                        <td className="py-2 text-sm text-right font-bold text-on-surface">{product.stockLevel}</td>
                                                        <td className="py-2 text-sm text-right">
                                                            <span className={`px-2 py-1 rounded-full text-xs ${(product.stockLevel || 0) === 0 ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'
                                                                }`}>
                                                ...
                                                {lowStockProducts.length === 0 && (
                                                    <tr>
                                                        <td colSpan={4} className="py-4 text-center text-on-surface-variant">All stock levels healthy</td>
                                                    </tr>
                                                )}
                                            </tbody>
                                        </table>

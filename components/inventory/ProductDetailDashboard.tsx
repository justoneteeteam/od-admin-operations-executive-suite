...
const ProductDetailDashboard: React.FC<ProductDetailDashboardProps> = ({ isOpen, product, onClose, onEdit }) => {
    ...
    const statusColor = (product as any).status === 'Active'
        ...
        : 'bg-gray-500/20 text-on-surface-variant border-gray-500/30';
    ...
    const formatTxType = (type: string) => {
        ...
        return map[type] || { label: type, color: 'text-on-surface-variant' };
    };
                ...
                <div className="px-6 py-4 border-b border-border-dark bg-surface-lowest flex-shrink-0">
                    <div className="flex items-start justify-between mb-3">
                        <div className="flex-1 min-w-0 pr-4">
                            ...
                            <div className="flex flex-wrap items-center gap-2 mt-2">
                                ...
                                {(product as any).category && (
                                    <span className="px-2.5 py-0.5 rounded text-[11px] font-bold uppercase tracking-wider bg-surface-high text-text-muted border border-border-dark">
                                        {(product as any).category}
                                    </span>
                                )}
                            </div>
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                            ...
                            <button
                                className="size-8 flex items-center justify-center rounded-lg hover:bg-surface-container text-text-muted transition-all ml-1"
                                title="More options"
                            >
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="p-6 space-y-6">
                                        ...
                                        <div className="mt-4">
                                            ...
                                            <div className="bg-surface-lowest rounded-xl border border-border-dark overflow-hidden">
                                                ...
                                                {stockSummary.warehouses.map((wh, idx) => {
                                                    ...
                                                    return (
                                                        <div key={idx} className="px-3 py-2.5 border-b border-border-dark/50 last:border-0 hover:bg-surface-high/30 transition-colors">
                                                            <div className="flex items-center justify-between">
                                                                <div className="flex items-center gap-2">
                                                                    <span className="material-symbols-outlined text-amber-400 text-[14px]">warehouse</span>
                                                                    <span className="text-xs text-on-surface font-medium">{wh.warehouseName}</span>
                                                ...
                                                <div className="flex items-center justify-between px-3 py-2.5 bg-surface-low border-t border-border-dark">
                                                    <span className="text-xs text-text-muted font-medium">Total (Parent)</span>
                                                    <span className="text-sm font-black text-on-surface">{onHand}</span>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <select
                                            ...
                                            className="bg-surface-high border border-border-dark rounded-lg px-2 py-1 text-on-surface text-xs"
                                        >
                                ...
                                {adsLoading ? (
                                    ...
                                    <div className="space-y-3">
                                        <div className="grid grid-cols-3 gap-2">
                                            {[
                                                ...
                                                <div key={i} className="bg-surface-lowest rounded-xl p-3 border border-border-dark">
                                                    <div className="text-[10px] font-bold text-text-muted uppercase tracking-wider">{k.label}</div>
                                                    <div className="text-lg font-black text-on-surface mt-0.5">{k.value}</div>
                                        ...
                                        <div className="space-y-0">
                                            {[
                                                { label: 'Total ad spend', value: `€${adsData.totalSpend.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, color: 'text-on-surface' },
                                                ...
                                                { label: 'ROAS', value: `${adsData.roas.toFixed(2)}×`, color: 'text-on-surface' },
                                                { label: 'CVR (lead → order)', value: `${adsData.cvr.toFixed(1)}%`, color: 'text-on-surface' },
                                            ].map((row, idx) => (
                                                <div key={idx} className="flex justify-between items-center py-2 border-b border-border-dark/50 last:border-0">
                                                    <span className="text-sm text-text-muted">{row.label}</span>
                                        ...
                                        <div>
                                            ...
                                            <div className="grid grid-cols-5 gap-2">
                                                {[
                                                    ...
                                                    { label: 'ORDERS', value: adsData.totalOrders, color: 'text-on-surface', sub: '' },
                                    ...
                                    <div className="bg-surface-lowest rounded-xl border border-border-dark p-6 text-center text-text-muted text-sm">
                    ...
                    {activeTab === 'warehouses' && (
                        <div className="p-6 space-y-4">
                            ...
                            {loading ? (
                                <div className="text-on-surface text-sm">Loading stock...</div>
                            ) : !stockSummary || stockSummary.warehouses.length === 0 ? (
                                <div className="bg-surface-lowest p-6 rounded-xl border border-border-dark text-center space-y-3">
                                ...
                                <div className="space-y-3">
                                    ...
                                    {/* Totals */}
                                    <div className="bg-surface-low rounded-xl px-4 py-3 border border-border-dark flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <span className="px-1.5 py-0.5 bg-primary/10 text-primary border border-primary/20 rounded text-[9px] font-bold">PARENT</span>
                                            <span className="text-sm text-text-muted font-medium">Total on-hand ({product.sku})</span>
                                        </div>
                                        <span className="text-xl font-black text-on-surface">{onHand}</span>
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    {activeTab === 'transactions' && (
                        <div className="p-6 space-y-4">
                            ...
                            {txLoading ? (
                                <div className="text-on-surface text-sm">Loading transactions...</div>
                            ) : transactions.length === 0 ? (
                                <div className="bg-surface-lowest p-6 rounded-xl border border-border-dark text-text-muted text-sm text-center">
                                    ...
                                    <table className="w-full text-left text-sm">
                                        ...
                                        <tbody className="divide-y divide-border-dark/50">
                                            {transactions.slice(0, 50).map(tx => {
                                                ...
                                                return (
                                                    <tr key={tx.id} className="hover:bg-surface-high/30 transition-colors">
                                                        ...
                                                        <td className="px-4 py-2 text-xs text-on-surface">{tx.warehouse?.name || '—'}</td>
                    ...
                    {activeTab === 'history' && (
                        <div className="p-6 space-y-4">
                            ...
                            <div className="bg-surface-lowest rounded-xl border border-border-dark p-10 text-center space-y-3">
                                <span className="material-symbols-outlined text-4xl text-text-muted/40">update</span>
                                <p className="text-on-surface font-bold text-sm">No History Records</p>
                                <p className="text-text-muted text-xs max-w-xs mx-auto leading-relaxed">
                                    Stock adjustments, price changes, and product edits will appear here automatically.
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="bg-surface-lowest p-6 rounded-xl border border-border-dark w-full max-w-md animate-in zoom-in-95 duration-200 shadow-2xl">
                        ...
                        {!needsFCPicker && (
                            <div className="bg-surface-container rounded-lg p-3 mb-4 border border-border-dark flex items-center gap-2">
                                <span className="material-symbols-outlined text-amber-400 text-[16px]">warehouse</span>
                                <span className="text-sm text-on-surface font-medium">{adjustWhName}</span>
                            ...
                            {[
                                ...
                                <button
                                    ...
                                    className={`flex-1 py-2.5 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-1 transition-colors ${
                                        adjustMode === m.mode ? 'bg-primary text-white' : 'bg-surface-high text-text-muted hover:text-on-surface'
                                    }`}
                                >
                ...
                <div className="fixed inset-0 z-[300] flex items-center justify-center">
                    ...
                    <div className="relative bg-[#0f1922] rounded-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden border border-border-dark shadow-2xl">
                        <div className="px-6 py-4 border-b border-border-dark bg-surface-lowest flex items-center justify-between">
                            ...
                            <button onClick={() => setBulkChildSkuOpen(false)} className="text-text-muted hover:text-on-surface transition-colors">
                                    ...
                                    {bulkChildSkus.map((row, idx) => (
                                        <tr key={row.warehouseId} className="border-t border-border-dark/50">
                                            <td className="py-2.5 pr-3">
                                                <div className="flex items-center gap-2">
                                                    <span className="material-symbols-outlined text-amber-400 text-[14px]">warehouse</span>
                                                    <span className="text-on-surface text-sm font-medium">{row.warehouseName}</span>
                                                </div>
                                            </td>
                                            <td className="py-2.5 pr-3">
                                                <input
                                                    ...
                                                    className="w-full bg-surface-high border border-border-dark rounded-lg px-2.5 py-1.5 text-on-surface text-xs focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none transition-colors"
                                            ...
                                            <td className="py-2.5">
                                                <input
                                                    ...
                                                    className="w-full bg-surface-high border border-border-dark rounded-lg px-2.5 py-1.5 text-on-surface text-xs font-mono focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none transition-colors"
